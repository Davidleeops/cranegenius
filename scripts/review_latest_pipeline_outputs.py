...
def build_report(args):
    ...
    if verify is None:
        ...
        valid    = verify.get("valid",    verify.get("emails_verified_valid",    "—"))
        catchall = verify.get("catchall", verify.get("emails_verified_catchall", "—"))
        invalid  = verify.get("invalid",  verify.get("emails_verified_invalid",  "—"))
        deferred = verify.get("deferred", verify.get("emails_deferred", "—"))
        total_v  = verify.get("total_sent", verify.get("total", "—"))
        lines.append(f"\nTotal sent to verifier: {total_v}")
        lines.append(f"  Valid    : {valid}")
        lines.append(f"  Catchall : {catchall}")
        lines.append(f"  Invalid  : {invalid}")
        lines.append(f"  Deferred : {deferred}")
        md.append(f"| Result | Count |\n|---|---|")
        md.append(f"| Total sent | {total_v} |")
        md.append(f"| Valid | {valid} |")
        md.append(f"| Catchall | {catchall} |")
        md.append(f"| Invalid | {invalid} |")
        md.append(f"| Deferred | {deferred} |\n")
